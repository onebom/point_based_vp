...
def draw_point_tracks(clips, pred_track, gt_track):
    ...
    pointed_clips=[]
    for f_idx in range(clips.size(1)):
        frame_gt, frame_pred = clip_gt[:,f_idx], clip_pred[:,f_idx] # C H W
        gt_point, pred_point = gt_track[:,f_idx], pred_track[:,f_idx] # PN C

        frame_gt_pointed = draw_points(frame_gt.cpu().numpy(), gt_point, colors = (255,0,0), interval=4) # c h w
        
        frame_pred_pointed = draw_points(frame_pred.cpu().numpy(), pred_point, colors = (255,0,0), interval=4)
        frame_pred_pointed = draw_points(frame_pred_pointed.transpose(2,0,1), pred_point, colors = (0,255,0), interval=4)

        clip_gt[:,f_idx] = frame_gt_pointed
        clip_pred[:,f_idx] = frame_pred_pointed
...
def draw_points(frame, track, colors, interval=None):
    ...
    if interval == None:
        ...
        points = points[::interval, ::interval] 
