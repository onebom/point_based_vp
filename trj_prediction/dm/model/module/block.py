...
class InitialBlock(nn.Module):
    def __init__(
        ...
        # traj
        self.coord_embedding = LinearEmbedding(ch, fea_dim)
    ...
    def forward(self, x, cond):
        ...
        cond = rearrange(cond, 'b c t pn -> b (t pn) c')
        x = self.coord_embedding(x)

        x = torch.concat([x, cond], dim=-1)
        x = self.trj_embedding(x)
        
        return rearrange(x, 'b (t pn) c -> b c t pn', t=T)
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, emb):
        ...
        frame_idx, time_rel_pos_bias = emb["frame_idx"], emb["time_rel_pos_bias"]

        x = self.trj_attnTemporal(x, pos_bias=time_rel_pos_bias)
        x = self.trj_attn2D(x, frame_idx = frame_idx)
        x = self.trj_attn(x)
...
class OutBlock(nn.Module):
    def __init__(
        ...
        self.trj_final_block = nn.ModuleList([LinearEmbedding(dim_in, dim_out, dropout_rate= 0.1)])
