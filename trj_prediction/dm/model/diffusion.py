...
class GaussianDiffusion(nn.Module):
    ...
    @torch.inference_mode()
    def sample(self, cond_x, gt_x, cond=None):
        noise = noise_sampling(shape = (gt_x.shape), device=gt_x.device, noise_cfg=self.noise_cfg)
        
        # sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sampling
        sample_fn = self.ddim_sampling
        return sample_fn(noise, cond_x, cond=cond)

    @torch.no_grad()
    def ddim_sampling(self, x_noise, cond_x, cond=None, clip_denoised=True):
        """
        param x: [B, C, Tf, H, W]
        ...
        for time, time_next in time_pairs:
            ...
            x = torch.concat((cond_x, x_noise), dim=2)
            time_cond = torch.full((batch,), time, device=device, dtype=torch.long)
            
            pred_x_noise = self.denoise_fn(x, time_cond, cond)
            ...
            x_out = x_start * alpha_next.sqrt() + c * pred_x_noise
    ...
    def p_losses(self, x_start_cond, x_start_pred, t, cond, noise = None, clip_denoised=True, **kwargs):
        b, c, f, pn, device = *x_start_pred.shape, x_start_pred.device
        
        x_start_pred=[]

        # noise = default(noise, lambda: torch.randn_like(x_start_pred))
        noise = noise_sampling(shape = (b, c, f, pn), device=device, noise_cfg=self.noise_cfg)
    ...
    def forward(self, x_cond, x_pred, cond=None, *args, **kwargs):
        b, device = x_cond.shape[0], x_cond.device
