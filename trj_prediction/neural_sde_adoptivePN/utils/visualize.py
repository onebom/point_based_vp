...
def visualize(save_path, pred, gt, frames, seg_map=None, score_info=None):
    ...
    if seg_map is not None:
        ...
        save_sample(save_root_path=save_path,index=index, 
                    pred=pred.cpu(), gt=gt.cpu(), frames=frames.cpu(), score_info=score_info)


def save_sample(save_root_path, index, pred, gt, frames, seg_map=None, score_info=None):
    ...
    for i in range(len(index)):
        ...
        track_seg = None
        if seg_map is not None:
            track_seg = seg_map[i] # H W

        for auto_idx in range(NUM_AUTOREG): # NUM_AUTOREG = 1
            ...
            if score_info is not None:
                track_score = score_info[i, auto_idx]

            pointed_clip = draw_point_tracks(three_clip, pred_track, gt_track, seg = track_seg, score = track_score) #3 T C H W

            #gif save
            gif=[]
            gif_clip = (pointed_clip*255).byte()
            for t in range(gif_clip.size(1)):
                gif_frame = [gif_clip[i,t].permute(1,2,0).cpu().numpy() for i in range(gif_clip.size(0))]
                combined_gif_frame = np.hstack(gif_frame)
                gif.append(Image.fromarray(combined_gif_frame))

            gif_save_path = os.path.join(save_root_path, "gif")
            os.makedirs(gif_save_path, exist_ok=True)
            gif[0].save(os.path.join(gif_save_path, f"gif_{index[i]}.gif"), save_all=True, append_images=gif[1:], duration=100, loop=0)
