...
class CityscapesDataset(object):
    def __init__(self, 
                 ...
                 num_observed_frames_val, num_predict_frames_val, wSeg=False                 
                 ):
        np.random.seed(0)
        self.wSeg = wSeg
        ...
        if self.wSeg:
            org_data_path = str(self.data_path)+"/"
            seg_data_path = org_data_path.replace('/leftImg8bit_sequence/', f'/gtFine/')
            self.val_video_data = self.__getVideoSegFolder__(os.path.join(seg_data_path,"val"))
        
        else: 
            val_video_paths = self.__getVideosFolder__(os.path.join(self.data_path,"val"))
    ...
    def __call__(self):
        ...
        if self.wSeg:
            val_dataset = ClipValSegDataset(self.num_observed_frames_val, 
