...
def save_sample(save_root_path, index, origin, result, save_pic, save_pic_row, save_gif, save_gif_grid, cond_frame_num, skip_pic_num, grid_nrow):
    
    if save_pic:
        save_pic_path = os.path.join(save_root_path, "pic")
    ...
    if save_gif:
        save_gif_path = os.path.join(save_root_path, "gif")
    ...
    if save_gif_grid:
        save_gif_grid_path = os.path.join(save_root_path, "gif_grid")
...
def save_sample_with_cond(save_root_path, index, origin, result, origin_motion_cond, motion_cond, motion_type, cond_frame_num, save_pic=False, save_pic_row=False):
    if save_pic_row:
        save_pic_row_path = os.path.join(save_root_path, "pic_row_w_cond")
        os.makedirs(save_pic_row_path, exist_ok=True)
        
        if motion_type=="point_track":
            B, NUM_AUTOREG, C, CFPF, PN = motion_cond.shape
            all_video = torch.stack([origin,result]) # 2 T C H W
            
            for i in range(len(index)):
                two_video = all_video[:,i] #2 T C H W
                video_point_track_origin = origin_motion_cond[i]
                video_point_track = motion_cond[i]
                
                
                c=CFPF-cond_frame_num
                for auto_idx in range(NUM_AUTOREG):
                    two_clip = two_video[:,c*auto_idx:c*auto_idx+CFPF] #2 CFPF C H W  #torch.Size([2, 6, 3, 64, 64])
                    
                    clip_point_track_origin = video_point_track_origin[auto_idx, :, :, :PN]
                    clip_point_track = video_point_track[auto_idx, :, :, :PN] # C CFPF PN//2
                    pointed_clip = draw_point_tracks(two_clip, clip_point_track, clip_point_track_origin) #2 CFPF c h w #torch.Size([2, 6, 3, 64, 64])
                    
                    all_clips = torch.stack([two_clip[0],pointed_clip[0]/255.0,pointed_clip[1]/255.0])  
                    # all_clips[-2:,:cond_frame_num] = 1.0
                    
                    all_clips = einops.rearrange(all_clips, "b t c h w -> (b h) (t w) c")
                    
                    save_path = os.path.join(save_pic_row_path, f"pic_row_{index[i]}")
                    os.makedirs(save_path, exist_ok=True)
                    media.write_image(os.path.join(save_path, f"autonum_{auto_idx}.png"), all_clips.squeeze().numpy())
            
        elif motion_type=="flow":
            B, NUM_AUTOREG, C, CFPF, H, W = motion_cond.shape
            flowed_origin=draw_flows(index, motion_cond.cpu().numpy())
            
            all_video = torch.stack([origin, torch.tensor(flowed_origin)/255.0, result])
            for i in range(len(index)):
                three_video = all_video[:, i]
                three_video[2, :cond_frame_num] = 1.0
                
                three_video = einops.rearrange(three_video, "b t c h w -> (b h) (t w) c")
                
                save_path = save_pic_row_path
                media.write_image(os.path.join(save_path, f"pic_row_{index[i]}.png"), three_video.squeeze().numpy())
            
    
def visualize(save_path, origin, result, origin_motion_cond=None, motion_cond=None, motion_type=None, epoch_or_step_num=0, cond_frame_num=10, skip_pic_num=1, save_pic_num=10, select_method='top', grid_nrow=4, save_pic=True, save_pic_row=True, save_gif=True, save_gif_grid=True):
    ...
    # 전부 다
    save_all_path = os.path.join(save_path, "all")
    os.makedirs(save_all_path, exist_ok=True)
    # epoch_or_step_save_all_path = os.path.join(save_all_path, f"{epoch_or_step_num}")
    # os.makedirs(epoch_or_step_save_all_path, exist_ok=True)

    index = [int(i) for i in range(len(origin))]
    save_sample(save_root_path=save_all_path, 
                index=index, origin=origin.cpu(), result=result.cpu(), 
                save_pic=save_pic, save_pic_row=save_pic_row, save_gif=save_gif, save_gif_grid=save_gif_grid, 
                cond_frame_num=cond_frame_num, skip_pic_num=skip_pic_num, grid_nrow=grid_nrow)
    
    if torch.is_tensor(motion_cond):
        save_sample_with_cond(save_root_path=save_all_path,
                            index=index, origin=origin.cpu(), result=result.cpu(),
                            origin_motion_cond = origin_motion_cond.cpu(),
                            motion_cond=motion_cond.cpu(), motion_type = motion_type, 
                            cond_frame_num=cond_frame_num, save_pic_row=True)

    # 가장 잘뽑힌 10개
    print("save", save_pic_num, "best samples")
        
    save_best_path = os.path.join(save_path, "result_best")
    os.makedirs(save_best_path, exist_ok=True)
    # epoch_or_step_save_best_path = os.path.join(save_best_path, f"{epoch_or_step_num}")
    # os.makedirs(epoch_or_step_save_best_path, exist_ok=True)
    ...
    save_sample(save_root_path=save_best_path, 
                index=index, origin=origin, result=result, 
                save_pic=save_pic, save_pic_row=save_pic_row, save_gif=save_gif, save_gif_grid=save_gif_grid, 
                cond_frame_num=cond_frame_num, skip_pic_num=skip_pic_num, grid_nrow=grid_nrow)
    
    if torch.is_tensor(motion_cond):
        motion_cond = motion_cond[index].cpu()
        save_sample_with_cond(save_root_path=save_best_path,
                            index=index, origin=origin.cpu(), result=result.cpu(),
                            origin_motion_cond = origin_motion_cond.cpu(),
                            motion_cond=motion_cond.cpu(), motion_type = motion_type, 
                            cond_frame_num=cond_frame_num, save_pic_row=True)
