...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, gt_frames, cond_frames, cond=None, cond_scale = 1.):
        ...
        return sample_fn(noise, cond_frames,(batch_size, channels, num_frames - cf, image_size, image_size), cond=cond)

    @torch.no_grad()
    def ddim_sampling(self, x_noisy, cond_frames, shape, cond=None, clip_denoised=True):
        ...
        for time, time_next in time_pairs:
            ...
            x = torch.concat((cond_frames, x_noisy), dim=2)
            
            pred_noise = self.denoise_fn(x, time_cond, cond=cond)
            x_start = self.predict_start_from_noise(x_noisy, t=time_cond, noise=pred_noise)
    ...
    def p_losses(self, x_start_cond, x_start_pred, t, cond = None, noise = None, clip_denoised=True, **kwargs):
        ...
        pred_noise = self.denoise_fn(x, t, cond=cond)
