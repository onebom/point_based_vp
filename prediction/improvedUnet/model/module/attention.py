...
class MoitonInteractMoudle(nn.Moudle):
    def __init__(
        self,
        dim,
        cond_dim=None,
    ...
    def forward(self, x, f_emb):
        B,C,T,PN = x.shape
        
        x = rearrange(x, "b c t pn -> b (pn t) c")
        f_emb = rearrange(f_emb, "b c t pn -> b (pn t) c")
        
        x = x + self.cross_attn(x, f_emb)
        x = x + self.cross_mlp(self.cross_norm(x))
        
        return rearrange(x, 'b (pn t) c -> b c t pn', pn=PN)
...
class MotionQEncoder(nn.Module):
    def __init__(
        ...
        # 2. motion encoder
        self.motion_encoder = MotionEncoder(in_dim=motion_dim, out_dim=dim_out)
