...
def draw_point_tracks( two_clips, clip_point_track):
    ...
    for f_idx in range(two_clips.size(1)):
        ...
        if f_idx < clip_point_track.size(1):
            ...
            pointed_origin = origin_frame.cpu().numpy().transpose(1,2,0)
            pointed_origin = (pointed_origin*255).astype(np.uint8)
            
            pointed_pred = pred_frame.cpu().numpy().transpose(1,2,0)
            pointed_pred = (pointed_pred*255).astype(np.uint8)
