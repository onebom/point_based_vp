...
from datasets.dataset import normalize_img, normalize_trj
    ...
    for epoch in range(epoch_cnt, cfg.train_params.max_epochs):
        for i_iter, batch in enumerate(train_loader):
            ...
            with accelerator.accumulate(model):                
                ...
                motion_cond[:,:2] = normalize_trj(motion_cond[:,:2]) #x,y

                with accelerator.autocast():
                    # template loss 사용안함. (=0.0)
                    loss = model(cond, gt, motion_cond)
                    
                    vid_loss, trj_loss = loss["vid_loss"], loss["trj_loss"]
                    total_loss = vid_loss + trj_loss
            ...
            logs = {'loss': total_loss.detach().item(), 
                    'vid_loss': vid_loss.detach().item(),
                    'trj_loss': trj_loss.detach().item(), 
...
def valid(cfg, accelerator, model, val_loader, motion_cond_predictor, global_step, vis=False):
    ...
    for i_iter, batch in enumerate(val_loader):
        ...
        for auto_step in tqdm(range(NUM_AUTOREG), desc='sampling loop'):
            ...
            if cfg.dataset.cond_params.cond_type:
                ...
                past_motion = motion_cond[:,:trj_dim,:cf].clone()
                original_motions.append(motion_cond[:,:trj_dim].clone().unsqueeze(1))
                
                motion_cond[:,:2] = normalize_trj(motion_cond[:,:2]) #x,y값만 normalize
            
            # B,C,T,H,W 
            pred_frames, pred_trj = model.sample_video(cond_frames = normalize_img(cond_frames),
            ...
            pred_trj = torch.stack(list(pred_trj[:,:trj_dim]), dim=0)
            pred_motion = torch.concat((past_motion, pred_trj), dim=2)
            result_motions.append(pred_motion.unsqueeze(1))
        ...
        if result_motions:
            ...
            original_motions = torch.cat(original_motions, dim=1)
            
            result_motion_cond.append(result_motions)
            origin_motion_cond.append(original_motions)
    ...
    if result_motion_cond:
        ...
        origin_motion_cond = torch.cat(origin_motion_cond, dim=0)
    ...
    # visulization
    # 1. hidden_f visulization
    # visualize_path = os.path.join(cfg.checkpoint.output, f'vdm_steps_{global_step}','hidden_features')
    # hidden_f_vis(hidden_for_debugging, visualize_path)
    # 2. result visulization 
    visualize_path = os.path.join(cfg.checkpoint.output, f'vdm_steps_{global_step}','vis_result')
    visualize(
        save_path=visualize_path,
        origin=origin_vids,
        result=result_vids,
        origin_motion_cond = origin_motion_cond,
