...
class VideoDirectDiffusion(nn.Module): 
    ...
    def forward(self, cond_frames, gt_frames, motion_cond=None, action=None):
        ...
        if self.unet_type in ["DirectUnet3D_CrossFrameAttn","Unet3D_noCond"]:
            ...
            #         "action": action}
            trj = motion_cond[:,:self.unet_cond_params.track_dim,:,:]
            expanded_trj = self.expand_motion_info(trj) 
            
            cond_trj = expanded_trj[:,:,:self.unet_params.cond_num]
            gt_trj = expanded_trj[:,:,self.unet_params.cond_num:]
            
            cond = {"traj_f": motion_cond[:,self.unet_cond_params.track_dim:, :self.unet_params.cond_num],
                    "action": action}
        
        diffusion_loss, _ = self.diffusion(cond_frames, gt_frames, cond_trj, gt_trj, cond = cond)
    ...
    def sample_video(self, cond_frames, gt_frames, motion_cond=None, action=None):
        ...
        # gt motion condition
        pred_vid, pred_trj = self.diffusion.sample(gt_frames, cond_frames, gt_trj, cond_trj, cond = cond)
        
        return pred_vid, pred_trj
