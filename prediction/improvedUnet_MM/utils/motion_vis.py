...
def draw_point_tracks( two_clips, clip_point_track, clip_point_track_origin):
    ...
    for f_idx in range(two_clips.size(1)):
        ...
        frame_point_origin = clip_point_track_origin[:, f_idx]
        
        pointed_origin = draw_points(origin_frame.cpu().numpy(), frame_point_origin, colors = (255,0,0), interval=4) # c h w
        
        pointed_pred = draw_points(pred_frame.cpu().numpy(), frame_point_origin, colors = (255,0,0), interval=4)
        pointed_pred = draw_points(pointed_pred.transpose(2,0,1), frame_point, colors = (0,255,0), interval=4)
    ...
    pointed_clips = pointed_clips.permute(1,0,2,3,4) #2 t c h w 
