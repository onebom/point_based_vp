...
import timeit
import os
# os.environ["CUDA_VISIBLE_DEVICES"]="MIG-66f2b9c3-bfb4-50d9-8927-22e78c404d2d"
import math
...
def parse_args():
    ...
    parser.add_argument('--config', default="./config/gt_pointTrack_guided_vp_ver2/cityscapes.yaml")
    ...
    for epoch in range(epoch_cnt, cfg.train_params.max_epochs):
        for i_iter, batch in enumerate(train_loader):
            ...
            with accelerator.accumulate(model):                
                cond, gt, temp_dist = batch["cond"], batch["gt"], batch["temp_dist"]
                cond, gt, temp_dist = cond.to(accelerator.device), gt.to(accelerator.device), temp_dist.to(accelerator.device)
                ...
                gt = normalize_img(gt)
                motion_cond = motion_cond/cfg.dataset.dataset_params.frame_shape ### flow to [-1,1](확인 필요), track to [0,1]

                with accelerator.autocast():
                    # template loss 사용안함. (=0.0)
                    diffusion_loss, template_loss = model(cond, gt, temp_dist, motion_cond)
                    total_loss = diffusion_loss + template_loss
            ...
            ### ::: 10. train logging         
            logs = {'total_loss': total_loss.detach().item(), 'diffusion_loss': diffusion_loss.detach().item(), 'template_loss': template_loss.detach().item(),
                    "lr": lr_scheduler.get_last_lr()[0], "epoch": epoch}
...
def valid(cfg, accelerator, model, val_loader, motion_cond_predictor, global_step, vis=False):
    ...
    for i_iter, batch in enumerate(val_loader):
        ...
        result_frames = []
        result_motions = []
        for auto_step in range(NUM_AUTOREG):
            ...
            if cfg.dataset.cond_params.cond_type: 
                # tracked_frames = torch.cat([cond_frames, origin_frames], dim=2)
                # tracked_frames = data[:,:,auto_step*pf:cf+auto_step*pf]
                tracked_frames = cond_frames
                
                motion_cond=create_motion_cond(tracked_frames, motion_cond_predictor, cfg.dataset.cond_params) #[b c cf+pf pn] or [b c cf+pf h w]
                motion_cond=motion_cond.to(accelerator.device)
    ...
    if cfg.dataset.cond_params.cond_type=="point_track":
        ...
        logs = {'pred_gif': wandb.Video(os.path.join(visualize_path,"all","gif","0","gif_result_0.gif"), fps=60, format="gif", width=300, height=300), 
                'gt_gif': wandb.Video(os.path.join(visualize_path,"all","gif","0","gif_origin_0.gif"), fps=60, format="gif", width=300, height=300)}
    accelerator.log(logs, step=global_step)
