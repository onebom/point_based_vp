...
def create_point_tack(videos, cond_predictor, cond_params):
    ...
    point_track = torch.empty(videos.size(0),videos.size(1),point_num*2,cond_params.point_track_params.track_dim, dtype=torch.float32)
    
    for i, t_idx in enumerate(cond_params.point_track_params.guery_frame_idx):
        # out: [b,t,point_num,2],[b,t,point_num]
        pred_tracks, pred_visibility = cond_predictor(videos, 
                                                        grid_size=cond_params.point_track_params.point_grid, 
                                                        grid_query_frame=t_idx) 
        point_track_info = torch.cat((pred_tracks, pred_visibility.float().unsqueeze(-1)), dim=3)
