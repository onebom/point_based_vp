...
def save_sample_with_cond(save_root_path, index, origin, result, motion_cond, motion_type, cond_frame_num, save_pic=False, save_pic_row=False):
    if save_pic_row:
        ...
        if motion_type=="point_track":
            ...
            for i in range(len(index)):
                ...
                for auto_idx in range(NUM_AUTOREG):
                    ...
                    clip_point_track = video_point_track[auto_idx, :, :, :PN//2] # C CFPF PN//2
                    pointed_clip = draw_point_tracks(two_clip, clip_point_track) #2 CFPF c h w #torch.Size([2, 6, 3, 64, 64])
                    
                    all_clips = torch.stack([two_clip[0],pointed_clip[0]/255.0,two_clip[1],pointed_clip[1]/255.0])  
...
def visualize(save_path, origin, result, motion_cond=None, motion_type=None, epoch_or_step_num=0, cond_frame_num=10, skip_pic_num=1, save_pic_num=10, select_method='top', grid_nrow=4, save_pic=True, save_pic_row=True, save_gif=True, save_gif_grid=True):
