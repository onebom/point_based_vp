...
class GaussianDiffusion(nn.Module):
    ...
    @torch.inference_mode()
    def sample(self, gt_frames, cond_frames, motion_cond=None, temporal_distance = None, cond=None, cond_scale = 1.):
        ...
        return sample_fn(noise, cond_frames, motion_cond, (batch_size, channels, num_frames - cf, image_size, image_size),
                         temporal_distance=temporal_distance, cond=cond, cond_scale = cond_scale)

    @torch.no_grad()
    def ddim_sampling(self, x, cond_frames, motion_cond, shape, temporal_distance=None, cond=None, cond_scale = 1., clip_denoised=True):
    ...
    def p_losses(self, x_start_cond, x_start_pred, motion_cond, temporal_distance, t, cond = None, noise = None, clip_denoised=True, **kwargs):
        ...
        pred_noise = self.denoise_fn(x_noisy, t, cond_frames=x_start_cond, motion_cond=motion_cond, temporal_distance=temporal_distance, 
                                     cond=cond, null_cond_prob=self.null_cond_prob, none_cond_mask=None, **kwargs)
    ...
    def forward(self, x_cond, x_pred, motion_cond=None, temporal_distance=None, cond=None, *args, **kwargs):
        b, device, img_size, = x_cond.shape[0], x_cond.device, self.image_size
        # check_shape(x, 'b c f h w', c = self.channels, f = self.num_frames, h = img_size, w = img_size)
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
        # x = normalize_img(x)
        return self.p_losses(x_cond, x_pred, motion_cond, temporal_distance, t, cond=cond, *args, **kwargs)