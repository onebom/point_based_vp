...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups=8, motion_dim=None,):
        super().__init__()
        self.proj = nn.Conv3d(dim, dim_out, (1, 3, 3), padding = (0, 1, 1))
        
        spade = True if exists(motion_dim) else False
        cond_dim = motion_dim if exists(motion_dim) else None
        
        self.norm = Normalization(dim_out, cond_dim=cond_dim, norm_type='group', num_groups=groups, spade=spade)
        self.act = nn.SiLU()
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim = None, groups=8, motion_dim=None):
        super().__init__()
        ...
        self.block1 = Block(dim, dim_out, groups=groups, motion_dim=motion_dim)
        self.block2 = Block(dim_out, dim_out, groups=groups, motion_dim=motion_dim)
