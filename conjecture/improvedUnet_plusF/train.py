...
from utils.visualize import visualize, hidden_f_vis
...
def parse_args():
    ...
    parser.add_argument('--config', default="./config/wo_motion_VP.yaml")
...
def train(cfg):
    ...
    if cfg.dataset.cond_params.cond_type == "point_track":
        motion_cond_predictor = torch.hub.load("facebookresearch/co-tracker", "cotracker2_offline")
...
def valid(cfg, accelerator, model, val_loader, motion_cond_predictor, global_step, vis=False):
    ...
    for i_iter, batch in enumerate(val_loader):
        ...
        for auto_step in tqdm(range(NUM_AUTOREG), desc='sampling loop'):
            ...
            # B,C,T,H,W 
            pred_frames, hidden_f = model.sample_video(cond_frames = normalize_img(cond_frames),
            ...
            hidden_features.append(hidden_f)
        ...
        hidden_for_debugging.append(hidden_features)
