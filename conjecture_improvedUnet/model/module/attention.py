...
class CondAttention2DModule(nn.Module):
    def __init__(
        ...
        if pn_prime and tc_dim:
            self.cross_attn = SpatialAttention(dim, cond_dim=dim, heads=heads, dim_head=dim_head, dropout=dropout)
            ...
            self.motion_query = MotionQAttention(pn_prime, tc_dim, num_motion_attn=num_motion_attn)
            self.motion_encoder = MotionEncoder(dim//4, in_dim=3, out_dim=dim)
            self.motion_mlp = Mlp(in_features=dim, hidden_features=int(mlp_ratio*dim), out_features=dim)
        
    def forward(self, x, motion_cond, frame_idx):
        B, C, T, H, W = x.shape
        x = rearrange(x, 'b c t h w -> (b t) (h w) c')
        ...
        if self.use_attn: #[32, 16]
            ...
            if motion_cond is not None:
                ...
                # 2. motion_cond embedding
                mc_out = self.motion_query(motion_cond) # out: [3,1024], [3,256] [3,64]
                mc = self.motion_encoder(mc_out) # out: [128,1024], [256, 256], [512, 64]
                # mc = mc + self.motion_mlp(mc)
            
                # 3. cross-attention
                mc = rearrange(mc, 'b c t pn -> (b t) pn c')
...
class MotionQAttention(nn.Module):
    def __init__(
        self,
        pn_prime,
        tc_dim,
        ...
        super().__init__()
        self.tc_dim = tc_dim
        self.query = nn.Parameter(torch.randn(pn_prime, tc_dim))
        
        self.cond_attn = nn.ModuleList([])
        for _ in range(num_motion_attn):
            self.cond_attn.append(nn.ModuleList([
                SpatialAttention(tc_dim, cond_dim=tc_dim, heads=heads, dim_head=dim_head, dropout=dropout),
                Normalization(tc_dim, norm_type='layer'), # norm_mlp
                Mlp(in_features=tc_dim, hidden_features=int(mlp_ratio*tc_dim), out_features=tc_dim)
            ]))
...
class MotionEncoder(nn.Module):
    def __init__(
        ...
        layers = []
        layers.append(nn.Conv2d(in_dim, ch, (1,5), padding = (0,2)))
        layers.append(nn.ReLU())

        while ch < out_dim:
            next_ch = min(ch * 2, out_dim) 
            layers.append(nn.Conv2d(ch, next_ch, (1,5), padding=(0,2)))
            layers.append(nn.ReLU())
            ch = next_ch
