...
class Unet3D_noCond(nn.Module):
    def __init__(
        ...
        last_resblock = partial(ResnetBlock, conv_method="2d", groups=resnet_groups)
        ...
        ### block
        now_res = frame_size
        cond_attn_res = attn_res # CondAttention: 32,16 | STWAttention: 4
        
        for ind, (dim_in, dim_out) in enumerate(in_out):
            is_last = ind >= (num_resolutions - 1)
            use_cond_attn = (now_res in cond_attn_res) 
            
            self.downs.append(nn.ModuleList([
                resblock2D(dim_in, dim_out, time_emb_dim = time_dim),
                resblockTemporal(dim_out, dim_out),
                CondAttention2DModule(dim_out, use_attn = use_cond_attn),
                CondAttentionTemporalModule(dim_out,
                                            heads = attn_heads,
                                            dim_head = attn_dim_head, 
                                            use_attn = use_cond_attn),
                AttentionSTModule(dim_out, use_attn = is_last),
                Downsample(dim_out) if not is_last else nn.Identity()
            ]))
            if not is_last:
                now_res = now_res // 2
        ...
        self.mid_sptmp_attn = AttentionSTModule(mid_dim, use_attn = True)
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                CondAttention2DModule(dim_in,use_attn = use_cond_attn),
                CondAttentionTemporalModule(dim_in, 
                                            heads = attn_heads,
                                            dim_head = attn_dim_head, 
                                            use_attn = use_cond_attn),
                AttentionSTModule(dim_in, use_attn = is_first),
                Upsample(dim_in, use_deconv, padding_mode) if not is_last else nn.Identity()
            ]))
    ...
    def traj_select(self, trj, traj_map, top_k=128):
        device = trj.device
        B, C, T, PN = trj.shape
        ...
        trj_topk_indices = topk_indices.unsqueeze(1).unsqueeze(1) # (B, 1, 1, 128)
        map_topk_indices = topk_indices.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)  # (B, 128, 1, 1, 1)
        
        selected_trj = torch.gather(trj, dim=-1, index=trj_topk_indices.expand(-1, C, T, -1))
        selected_traj_map = torch.gather(traj_map, dim=1, index=map_topk_indices.expand(-1, -1, T, H, W))
        
        return selected_trj, selected_traj_map
    ...
    def forward(self, x, time, cond_frames, cond = None):
        ...
        # ### 2. select trajectory
        # mc, mc_map = self.traj_select(trj, trj_map, self.top_k) # [b,c,t,pn], [b,c,t,h,w]
        # mc_map = self.mc_init_conv(mc_map)
        # mc_map = self.mc_init_tmp_attn(mc_map, pos_bias=time_rel_pos_bias)
        mc, mc_map = None, None
        ...
        h = []
        ###### 4-1. down layers
        for res, tmp_res, attn, tmp_attn, sptmp_attn, downsample in self.downs:
            x = res(x, t)
            x = tmp_res(x)
            x, mc = attn(x, mc, frame_idx=frame_idx)
            x = tmp_attn(x, mc_map, pos_bias=time_rel_pos_bias)
        ...
        x = self.mid_sptmp_attn(x, frame_idx=frame_idx)
        
        
        ###### 4-3. up layers
        for res, tmp_res, attn, tmp_attn, sptmp_attn, upsample in self.ups:
            x = torch.cat((x, h.pop()), dim=1)
            x = res(x, t)
            x = tmp_res(x)
            x, mc = attn(x, mc, frame_idx=frame_idx)
...
class Unet3D_SequentialCondAttn(nn.Module):
    def __init__(
        ...
        # 1. mcmap init conv & tmp_attn
        self.mc_init_conv = nn.Conv3d(self.top_k, init_dim,
                                      (1, init_kernel_size, init_kernel_size),
                                      padding = (0, init_padding, init_padding))
        self.mc_init_tmp_attn = temporal_attn(init_dim)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            is_last = ind >= (num_resolutions - 1)
            use_mc_res, use_cond_attn = (now_res >= cond_attn_res[-1]), (now_res in cond_attn_res) 
            
            self.downs.append(nn.ModuleList([
                resblock2D(dim_in, dim_out, time_emb_dim = time_dim),
                resblockTemporal(dim_out, dim_out),
                CondAttention2DModule(dim_out, 
                                      pn_prime=now_res**2,
                                      tc_dim=(self.tc+self.tp)*self.track_dim,
                                      use_attn = use_cond_attn),
                resblock2D(dim_in, dim_out, 
                           use_res = use_mc_res), # for mcmap
                resblockTemporal(dim_out, dim_out, 
                                 use_res = use_mc_res), # for mcmap
                CondAttentionTemporalModule(dim_out,
                                            heads = attn_heads,
                ...
                AttentionSTModule(dim_out, use_attn = is_last),
                Downsample(dim_out) if not is_last else nn.Identity(),
                Downsample(dim_out) if use_mc_res else nn.Identity()                
            ]))
            if not is_last:
                now_res = now_res // 2
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            is_last = ind >= (num_resolutions - 1)
            use_mc_res, use_cond_attn = (now_res >= cond_attn_res[-1] and not is_last), (now_res in cond_attn_res)
            is_first = (ind ==0)
            
            self.ups.append(nn.ModuleList([
                resblock2D(dim_out*2, dim_in, time_emb_dim = time_dim),
                resblockTemporal(dim_in, dim_in),
                CondAttention2DModule(dim_in, 
                                      pn_prime=now_res**2,
                                      tc_dim=(self.tc+self.tp)*self.track_dim,
                                      use_attn = use_cond_attn),
                resblock2D(dim_out, dim_in, 
                           use_res = use_mc_res), # for mcmap
                resblockTemporal(dim_in, dim_in, 
                                 use_res = use_mc_res), # for mcmap
                CondAttentionTemporalModule(dim_in, 
    ...
    def forward(self, x, time, cond_frames, cond = None):
        ...
        h = []
        ###### 4-1. down layers
        for res, tmp_res, attn, mcmap_res, mcmap_tmp_res, tmp_attn, sptmp_attn, downsample, downsample_mc in self.downs:
            x = res(x, t)
            x = tmp_res(x)
            x, mc = attn(x, mc, frame_idx=frame_idx)
            mc_map = mcmap_res(mc_map)
            mc_map = mcmap_tmp_res(mc_map)
            x = tmp_attn(x, mc_map, pos_bias=time_rel_pos_bias)
            x = sptmp_attn(x, frame_idx=frame_idx)
            h.append(x)
            x = downsample(x)
            mc_map = downsample_mc(mc_map)
        ...
        ###### 4-3. up layers
        for res, tmp_res, attn, mcmap_res, mcmap_tmp_res, tmp_attn, sptmp_attn, upsample, upsample_mc in self.ups:
            x = torch.cat((x, h.pop()), dim=1)
            x = res(x, t)
            x = tmp_res(x)
            x, mc = attn(x, mc, frame_idx=frame_idx)
